...
class MultiWindowManager : public std::enable_shared_from_this<MultiWindowManager>, public FlutterWindowCallback {
 ...
 public:
  ...
  void Show(int64_t id, bool isNotification);
  ...
  void SetNewReact(int64_t id, double_t width, double_t height);

  void SetOpacity(int64_t id, double_t opacity);

  void SetBoundsWithAnimation(int64_t id, double_t x, double_t y, double_t width, double_t height);

  flutter::EncodableMap GetBounds(int64_t id, double_t devicePixelRatio);

  flutter::EncodableList GetAllSubWindowIds();

  void Focus(int64_t id);

  void UpNotification(int64_t id);

  void DownNotification(int64_t id);
